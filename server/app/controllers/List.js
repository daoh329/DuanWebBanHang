  ...
  async add(req, res) {
    try {
      ...
      if (table == "capacity") {
        ...
        const sql = `INSERT INTO ${table} (capacity) VALUES (?)`;
        mysql.query(sql, [capacity.capacity], (error, results) => {
          if (error) {
    ...
    try {
      ...
      mysql.query(sql, req.body, (error, results) => {
        if (error) {
          ...
          return res
            .status(500)
            .json({ message: "Đã xảy ra lỗi khi sửa danh mục" });
        }
        ...
        return res
          .status(200)
          .json({ message: "Danh mục đã được sửa thành công" });
      });
    } catch (error) {
      console.error("Lỗi:", error);
      return res
    ...
    try {
      ...
      mysql.query(sql, (error, results) => {
        if (error) {
          ...
          return res
            .status(500)
            .json({ message: "Đã xảy ra lỗi khi lấy danh mục" });
        }
      ...
      return res
        .status(500)
        .json({ message: "Đã xảy ra lỗi khi lấy tất cả danh mục" });
    }
  }
}
