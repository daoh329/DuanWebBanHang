...
function OrderList() {
  ...
  const loadData = () => {
    axios
      .get(`${process.env.REACT_APP_API_URL}/order/quanlyOrder`)
      .then((res) => {
  ...
  const handleConfirmOrder = async (record) => {
    if (record.order_id) {
      console.log("Confirm order button clicked for order:", record);
      try {
        await axios.put(
        ...
        CreateNotification(
          ...
          `Đơn hàng ${record.order_id} của bạn đã được xác nhận, sản phẩm đang trong quá trình đóng gói và giao cho đơn vị vận chuyển`,
        );
      } catch (error) {
  ...
  return (
    <div>
      ...
      <div>
        <a
          href="https://sandbox.vnpayment.vn/merchantv2/Transaction/SearchRefund.htm"
          style={{
            width: 250,
            height: 40,
            marginTop: "10px",
