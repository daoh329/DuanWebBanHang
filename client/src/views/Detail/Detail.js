...
const provinces = [
  {
    id: 1,
    name: 'Tỉnh A',
    districts: ['Huyện A1', 'Huyện A2', 'Huyện A3'],
  },
  {
    id: 2,
    name: 'Tỉnh B',
    districts: ['Huyện B1', 'Huyện B2', 'Huyện B3'],
  },
  // ...
];
...
function Detail() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const res = await axios.get(`https://64df1e7171c3335b25821aef.mockapi.io/users/${id}`);
  ...
  const handleProvinceChange = (value) => {
    setSelectedProvince(provinces.find((province) => province.id === value));
