...
const Search = () => {
    ...
    return (
        <div style={{ borderRadius: '20px', width: '80%', margin: '0 auto', marginTop: '20px', backgroundSize: 'cover', backgroundColor: "white" }}>
            <div>
                <div style={{ fontWeight: 'bold', fontSize: '25px', display: '-webkit-box', padding: '20px', color: 'black' }}>Kết quả tìm kiếm</div>
            </div>
                ...
                {filteredProducts && filteredProducts.length > 0 &&
                    filteredProducts.slice(startIndex, endIndex).map((product, index) => (
                        <Card
                            ...
                            className="card-sp"
                        >
                            <img src={product.avatar} style={{ width: '170px', height: '170px', objectFit: 'cover' }} alt={product.name} onClick={() => handleViewDetailuser(product)} />
